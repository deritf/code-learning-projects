...
if (isset($_POST["submit"])) {
    ...
    if (!empty($_POST["password"]) && strlen($_POST["password"]) >= 6) {
        ...
        $errors["password"] = "Introduce una contraseña válida.";
    }
    ...
    if (count($errors) == 0) {
        ...
        $sql = "INSERT INTO users (name, surname, bio, email, password, role, image)
                VALUES ('$name', '$surname', '$bio', '$email', '$hashed_password', '$role', " . ($image ? "'$image'" : "NULL") . ")";
...
<form action="crear.php" method="POST" enctype="multipart/form-data">
    ...
    <select name="role" id="role">
        <option value="" <?php if (!isset($_POST["role"]) || $_POST["role"] === "") echo "selected"; ?>>Selecciona un rol</option>
